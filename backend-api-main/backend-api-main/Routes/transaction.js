...
let UPIGatewaySecretKey = configFile.get('general.UPIGatewayKey');
let paymentRedirectUrl = configFile.get('general.paymentRedirectUrl');
...
async function getRazorpayKeys() {
    try {
        ...
        console.error('Error fetching Razorpay keys2:', error);
...
const upigatewayKey = UPIGatewaySecretKey;
...
router.post('/razorpaycheck/response', Auth, async(req, res) => {
    ...
    if (txn.order_id === orderID && txn.order_token === orderToken && txn.status != 'PAID') {
        try {
            ...
            var instance = new Razorpay({ key_id: razorpayKeys.key, key_secret: razorpayKeys.secret })
...
router.post('/razorpaydesposit/response', Auth, async(req, res) => {
    ...
    if (txn.order_id === orderID && txn.order_token === orderToken && txn.status != 'PAID') {
        try {
            ...
            const options = {
                method: 'POST',
                url: `https://${razorpayKeys.key}:${razorpayKeys.secret}@api.razorpay.com/v1/payments/${rpPaymentId}/capture`,
                form: {
                    amount: Math.round(txn.amount * 100), // amount must be integer
                    currency: "INR",
                }
            };
...
router.post("/user/depositeupi", Auth, async(req, res) => {
    ...
    await axios.post('https://merchant.upigateway.com/api/create_order', {
        ...
        console.log("deosit", result.data)
        if (result.data.status) {
            ...
            res.status(400).send({ data: result.data, error: result.data.msg || "Failed to create transaction." });
        }
    }).catch((e) => {
        console.log(e)
...
router.post('/depositupipay/response', Auth, async(req, res) => {
    console.log("deposit/response called");
    ...
    if (txn.order_id === orderID && txn.order_token === orderToken.toString() && txn.status != 'PAID') {
        ...
        if (user.withdraw_holdbalance > 0 && type == 'upi') {
            ...
            if (InProcessSubmit == false) {
                ...
                if (txn.status === 'SUCCESS' || txn.status === 'FAILED') {
                    ...
                    // Get dynamic Razorpay keys
                    const razorpayKeys = await getRazorpayKeys();
                    var username = razorpayKeys.key;
                    var password = razorpayKeys.secret;
                    (
                        async() => {
                            const axios = require('axios').default;
                            const options = {
    ...
    if (withdraw && withdraw.status == 'Pending') {
        ...
        if (txn._id == orderID && txn.referenceId == referenceId && txn.status != 'SUCCESS') {
            try {
                ...
                const options = {
                    method: 'GET',
                    url: `https://${razorpayKeys.key}:${razorpayKeys.secret}@api.razorpay.com/v1/payouts/${referenceId}`
                };
                axios.request(options).then(function(response) {
