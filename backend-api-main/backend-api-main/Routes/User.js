...
const { sendOTP, verifyCredentials } = require('../services/twilioSMS');
const smsHubIndia = require('../services/smsHubIndia');
...
router.post("/agent/permission/nested/:id", Auth, async(req, res) => {
    try {
        const updateResult = await User.findOneAndUpdate({ 'Permissions._id': req.params.id }, { 'Permissions.$.Status': req.body.Status });
...
router.post("/login", async(req, res) => {
    try {
        ...
        if (user != null) {
            ...
            // Send OTP via SMSHub India (Primary)
            try {
                const smsResult = await smsHubIndia.sendOTP(Phone, newSecret.token);
                if (smsResult.success) {
                    console.log(`✅ OTP sent successfully to ${Phone} via SMSHub India`);
                } else {
                    console.error(`❌ Failed to send OTP via SMSHub India: ${smsResult.error}`);
                    // Fallback to Twilio
            ...
            // Send OTP via SMSHub India for new user
            try {
                const smsResult = await smsHubIndia.sendOTP(Phone, newSecret.token);
                if (smsResult.success) {
                    console.log(`✅ OTP sent successfully to new user ${Phone} via SMSHub India`);
                } else {
                    console.error(`❌ Failed to send OTP via SMSHub India: ${smsResult.error}`);
                    // Fallback to Twilio
                    await sendOTP(Phone, newSecret.token);
        ...
        console.log("token", newSecret.token);
    } catch (e) {
        res.status(400).send(e)
        console.log(e);
    }
    ...
    try {
        ...
        if (!user) {
            return res.status(401).json({
                status: 101,
                msg: "Admin/Agent not found with this phone number"
            });
        }
        ...
        try {
            const smsResult = await smsHubIndia.sendOTP(phone, newSecret.token);
            if (smsResult.success) {
                console.log(`✅ Admin OTP sent successfully to ${phone} via SMSHub India`);
            } else {
                console.error(`❌ Failed to send admin OTP via SMSHub India: ${smsResult.error}`);
                // Fallback to Twilio
                await sendOTP(phone, newSecret.token);
            }
        } catch (smsError) {
...
router.get('/website/setting', async(req, res) => {
    ...
    // Add null/undefined check for paymentGateway
    const webSetting = {
        ...
        'isUpiGatewayActive': paymentGateway && paymentGateway.UPIEnabled !== undefined ? paymentGateway.UPIEnabled : true,
        'isManualPayoutActive': true,
        'isCashFreePayoutActive': false,
        'isRazorPayPayoutActive': paymentGateway && paymentGateway.RazorPayout ? paymentGateway.RazorPayout : false,
        'isDecentroPayoutActive': paymentGateway && paymentGateway.decentroPayout ? paymentGateway.decentroPayout : false,
        'maxAutopayAmt': 1500,
        'RazorPayKey': paymentGateway && paymentGateway.RazorPayKey ? paymentGateway.RazorPayKey : (process.env.RAZORPAY_KEY_ID || 'rzp_test_wEwkyVMPi3twxg'),
        'RazorPaySecretKey': paymentGateway && paymentGateway.RazorPaySecretKey ? paymentGateway.RazorPaySecretKey : (process.env.RAZORPAY_KEY_SECRET || 'oUEa4OM4BwlJ1C64izsWieJz'),
        'AccountName': paymentGateway && paymentGateway.AccountName ? paymentGateway.AccountName : '',
        'isDecentroPayoutAuto': paymentGateway && paymentGateway.decentroAuto ? paymentGateway.decentroAuto : false,
        'isRazorPayPayoutAuto': paymentGateway && paymentGateway.RazorpayAuto ? paymentGateway.RazorpayAuto : false,
        'UPIGatewayKey': paymentGateway && paymentGateway.UPIGatewayKey ? paymentGateway.UPIGatewayKey : '',
        'UPIGatewaySecret': paymentGateway && paymentGateway.UPIGatewaySecret ? paymentGateway.UPIGatewaySecret : '',
    }
    res.send(webSetting)
})

// Twilio Configuration Routes
router.get("/twilio/status", Auth, RoleBase, async (req, res) => {
    try {
        const credentialsValid = await verifyCredentials();
		console.log(credentialsValid);
        res.json({
            success: true,
            twilioConfigured: credentialsValid,
            message: credentialsValid ? "Twilio is properly configured" : "Twilio credentials not configured"
        });
    } catch (error) {
        res.status(500).json({
            success: false,
...
router.post("/twilio/test", Auth, RoleBase, async (req, res) => {
    try {
        ...
        const smsResult = await sendOTP(phoneNumber, testOTP, "This is a test SMS from Twilio. Your test OTP is: 123456");
