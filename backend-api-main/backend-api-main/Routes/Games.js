...
router.post("/challange/create", Auth, async (req, res) => {
    ...
    try {
        ...
        if (Game_Ammount >= 10 && Game_Ammount <= 15000) {
            if (Game_Ammount % 10 === 0) {
...
async function adminProfit(gameAmount, winID) {
    ...
    let referralPer = 0;
    if (winner.referral) {
        referralPer = userReferralPer;
        console.log("referralPer", referralPer);
        ...
        referralTxn.amount = gameAmount * (referralPer / 100);
        referralUser[0].referral_earning += gameAmount * (referralPer / 100);
        referralUser[0].referral_wallet += gameAmount * (referralPer / 100);
