    ...
    axios.get(baseUrl + `me`, {
      ...
      if (res.data.user_type === "Admin") {
        ...
        this.setState({
          usertype: "Agent",
          userAname: res.data.Name,
          dashboard: res.data.user_type === "Agent" ? res.data.Permissions[0]?.Status : false,
          ...
          sitesettings: res.data.Permissions[27]?.Status || false, // Added sitesettings for Agent
          twilioConfig: res.data.Permissions[28]?.Status || false, // Added twilioConfig for Agent
        });
      }
    }).catch(err => {
      console.log(err);
