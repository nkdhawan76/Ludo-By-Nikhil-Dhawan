...
const TwilioConfig = lazy(() => import('./components/adminManagement/TwilioConfig'));
...
const AppRoutes = () => {
  ...
  const [twilioConfig, setTwilioConfig] = useState(false)
    ...
    if (access_token) {
      axios.get(baseUrl+`me`, {
        ...
        if(res.data.user_type === "Agent"){
          ...
          setTwilioConfig(res.data.Permissions[27].Status)
      ...
      <>
        ...
        <Suspense fallback={<Spinner />}>
          <Switch>
            ...
            {twilioConfig && <Route path="/admin/twilio-config" component={TwilioConfig} />}
