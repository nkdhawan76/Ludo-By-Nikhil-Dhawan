...
const Addcase = ({ walletUpdate }) => {
  ...
  async function displayRazorpayPaymentSdk(channel, method, upiMethod, razorpay) {
    ...
    // Add check for orderdata and orderdata.id
    if (!response.data.orderdata || !response.data.orderdata.id) {
      Swal.fire({
        title: response.data.message || 'Failed to create Razorpay order',
        icon: 'error',
        confirmButtonText: "OK",
      });
      return;
  ...
  const phonePeDepositeUpi = (channel, method, upiMethod, phonepay) => {
    ...
    axios.post(baseUrl + `user/phonedepositeapi`,
      { amount: global, channel: channel, payment_method: method, provider: walletOption, bankCode: bankCode, account_name: account_name, payment_gatway: phonepay },
      { headers })
      .then((res) => {
  ...
  const depositUpiGateway = (channel, method, upiMethod, upigateway) => {
    ...
    axios.post(baseUrl+`user/depositeupi`,
      { amount: global, channel: channel, payment_method: method, provider: walletOption, bankCode: bankCode, account_name: account_name || 'dsfsd', account_mail_id: account_mail_id || 'dsfsd', payment_gatway: upigateway },
      { headers })
      .then((res) => {
  ...
  >
    <span style={{ fontSize: '18px', color: '#007BFF', marginRight: '8px' }}>
      ℹ️
    </span>
    <span>
      Deposit कैसे करे इसका वीडियो Youtube चैनल पर जाके देखें{' '}
      <a
        href="#"
        style={{
  ...
  >
    <span style={{ fontSize: '18px', color: '#FFA500', marginRight: '8px' }}>
      ⚠️
    </span>
    बिना KYC किये आप ONLY 500 तक ही ADD कर सकते हैं
    </div>
              <div className="pb-3">
              ...
              <div className={`${css.refer_footer}`}>
                <div className="d-grid gap-2 col-12 mx-auto">
                  ...
                  <button type="button" className={`${css.block} bg-primary rounded text-white font-weight-bold text-uppercase`} onClick={() => { (global >= 1) ? setNext(2) : alert('Minimum deposit is 10') }}>Next</button>
            ...
            {Boolean(next === 2) && <div className="px-4 py-3">
              ...
              <div
                    onClick={() => {
                      // Validation before calling depositUpiGateway
                      // if (!account_name || !account_mail_id || !global) {
                      //   Swal.fire({
